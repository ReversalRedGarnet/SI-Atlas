 ...
 *     Guadalcanal (fileId 5491) and Honiara (fileId 5510) Year 10 Placement
 *     2026 folders. Investigated directly: both PDFs list the exact same 22
 *     named students, same student codes, same order — not two different
 *     cohorts — generated by the same MEHRD author roughly 65 minutes apart
 *     on the same day (2026-01-15), confirming this is one school's roster
 *     duplicated into both folders, not two coincidentally same-coded
 *     schools. No independent source (news coverage, a church record, a
 *     business directory) could be found anywhere for a "Gospel Light CHS,"
 *     so which province is authoritative still can't be determined. Left
 *     out of this dataset rather than guessing.
 *   - The "188" Norman Palmer CHS / Christ The King CHS conflict:
 *     Norman Palmer CHS's own record in this file is independently
 *     confirmed (real, Anglican, operating in East Honiara today).
 *     Investigated further: MEHRD's own code-188 document dated 2023-02-03
 *     names Norman Palmer CHS; MEHRD's own current code-188 document
 *     (dated 2026-01-15) names Christ The King CHS instead — a code
 *     reassignment over time, not a same-moment collision, the same
 *     pattern that resolved St Johns Bosco/St Johns Nila CHS's code 508
 *     below. Unlike that case, though, this isn't a rename: Norman Palmer
 *     CHS demonstrably still exists under its own name in 2026, so
 *     whatever Christ The King CHS is, it isn't Norman Palmer relabeled.
 *     Christ The King CHS itself could not be independently confirmed —
 *     it does not appear in MEHRD's own current Guadalcanal or Honiara
 *     provincial listing pages, and no news coverage, social-media page or
 *     diocesan record turned up. The nearest lead, a real Catholic "Church
 *     of Christ the King" at Tetere, Guadalcanal, is a single unconfirmed
 *     hit with no source tying it to a school, so it wasn't acted on. No
 *     record added for Christ The King CHS; flagged here rather than
 *     guessed.
...
SF.SCHOOLS = [
  ...
  },
  {
    ...
    "description": "Bishop Norman Palmer Anglican School, an Anglican Church of Melanesia (ACOM)-owned community school in East Honiara, offering secondary education. MEHRD's own code-188 document from 2023 names this school; MEHRD's own current (2026) code-188 document instead names a \"Christ The King CHS\" — a code reassignment over time, not a live collision, since this school is independently confirmed alive and operating under its own name today. Christ The King CHS could not itself be independently confirmed or located anywhere. Left unrenamed here; see this file's DATASET CAVEATS section or git history for the full investigation.",
    ...
    "sourceUrls": [
      ...
      "https://solomons.gov.sb/wp-content/uploads/2023/02/188-Norman-Palmer-CHS-1.pdf",
      "https://mehrd.gov.sb/documents?fileId=5504&format=raw&view=download"
    ],
